...
class Py3status:
    ...
    def post_config_hook(self):
        ...
        self.headers = {"Client-ID": self.client_id, "Authorization": f"Bearer {self.bearer}"}
        self.stream_url = "https://api.twitch.tv/helix/streams/"
        self.user_url = f"https://api.twitch.tv/helix/users/follows?from_id={self.user_id}"
    ...
    def _get_users(self):
        ...
        while data["pagination"].get("cursor"):
            data = self._get_twitch_data(self.user_url + f"&after={data['pagination']['cursor']}")
            users += [u["to_id"] for u in data.get("data")]
            if len(users) >= 100:
                break
        return users[:99]
    ...
    def twitch(self):
        ...
        if self.streams:
            ...
            return {
                ...
                "full_text": self.py3.safe_format(self.format, {"live_count": len(self.streams)}),
            }
        return {
            "cached_until": self.py3.time_in(self.cache_timeout),
            "full_text": "",
    ...
    def on_click(self, event):
        ...
        if event["button"] == 1:
            ...
            notification_message += "\n".join([f"{s['name']} with {s['game']} ({s['viewers']})" for s in self.streams])
